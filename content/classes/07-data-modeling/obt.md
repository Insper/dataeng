...
Na [aula 03](../03-data-warehouse/pratica.md), quando fizemos a implementação do nosso **Data Warehouse (DW)**, foi preciso decidir como organizar os dados no **DW**.
...
- **Performance de Consultas**: A principal vantagem do OBT é a **eliminação de JOINs** complexos. Em um **DW** tradicional, uma consulta simples pode requerer junções entre múltiplas tabelas, criando gargalos de performance.

- **Simplicidade de Consultas**: Analistas e cientistas de dados podem escrever consultas mais diretas, sem precisar compreender relações complexas entre tabelas.
...
- **Redundância de Dados**: A desnormalização resulta em **duplicação massiva** de informações. Dados que antes apareciam uma única vez em tabelas normalizadas agora são replicados em várias linhas da **OBT**.
...
A decisão de implementar uma **OBT** deve considerar diversos fatores:
...
Vamos trabalhar com a transformação do nosso modelo de vendas normalizado em uma **OBT**. 

Antes, vamos relembrar a estrutura original:
